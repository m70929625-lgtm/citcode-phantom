...
router.get('/', (req, res) => {
    try {
        const anomalies = queryAll(`
            ...
            WHERE recommended_action IN ('STOP_INSTANCE', 'SET_LIFECYCLE')
            AND status = 'new'
        ...
        const totalSavings = recommendations.reduce((sum, r) => sum + (r.monthlySavings || 0), 0);
...
router.post('/execute', (req, res) => {
    try {
        const { anomalyId, resourceId, resourceName, resourceType, action, monthlySavings, confidence } = req.body;
        ...
        const actionData = queryOne('SELECT * FROM actions WHERE id = ?', [actionId]);
